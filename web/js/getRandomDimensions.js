...
function add_autoprompts() {
  ...
  ComfyWidgets.STRING = function (node, inputName, inputData, app) {
      if (app.ui.settings.getSettingValue('AE.autoprompt')) {
...
function addNewColors() {
  console.log(`Adding new colors executed: ${app.ui.settings.getSettingValue('ACC.colors')}`)
  if (app.ui.settings.getSettingValue('ACC.colors')) {
    // Completely replacing the node_colors object
    console.log("Adding new colors condition passed")
...
app.registerExtension({
  ...
  setup() {
      app.ui.settings.addSetting({
        id: "ACC.colors",
        name: "Accessories show material colors",
        type: "boolean",
        defaultValue: false,
        // options: [
        //   { value: true, text: "On" },
        ...
        // ],
        onChange: (value) => {
          console.log(`Captured activate colors: ${value}`)
          addNewColors();
        }
     });
